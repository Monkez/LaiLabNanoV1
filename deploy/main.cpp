...
int init_metadata_socket() {
    ...
    // Use subnet broadcast instead of global broadcast (INADDR_BROADCAST) to ensure
    // it goes over the USB RNDIS network (192.168.100.x) instead of getting lost.
    metadata_broadcast_addr.sin_addr.s_addr = inet_addr("192.168.100.255");
    
    printf("[META] UDP metadata broadcast on port %d\n", METADATA_PORT);
    return 0;
}
